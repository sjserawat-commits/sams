...
export default function SlipPrintSearchPage() {
  ...
  const matches = useMemo(() => {
    ...
    if (!q) return [];
    return patients.filter(p => `${p.firstName} ${p.lastName || ""}`.toLowerCase().includes(q) || String(p.patientId || "").toLowerCase().includes(q));
  }, [patients, query]);
  ...
  return <main className="min-h-screen bg-[#f4f7fb] text-slate-900">
    <header className="border-b border-[#d6a443]/30 bg-[#061525] px-4 py-4 text-white sm:px-8"><div className="mx-auto flex max-w-[1200px] items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.25em] text-[#d6a443]">SAMS · OPD</p><h1 className="mt-1 text-2xl font-black">OPD Slip Print</h1></div><button onClick={() => router.push("/doctor-desk")} className="rounded-xl border border-white/15 bg-white/10 px-4 py-2 text-xs font-black">← Doctor Desk</button></div></header>
    <div className="mx-auto max-w-[1200px] px-4 py-7 sm:px-8">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,38,70,.10)] sm:p-8">
        <div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#0b63ce]">Patient Search</p><h2 className="mt-1 text-2xl font-black text-[#082b61]">Search patient for OPD slip</h2><p className="mt-1 text-sm font-semibold text-slate-500">Search by Patient Name or HID.</p></div>
