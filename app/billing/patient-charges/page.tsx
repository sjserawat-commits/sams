...
export default function PatientChargesPage(){
  ...
  const load=async(id:number)=>{
    ...
    try{
      ...
      // Patient Charges owns the charge-building stage. New/old unpaid bills receive
      // the OPD consultation fee automatically instead of asking staff to type it.
      if(d.bill && !d.bill.paidAmount && d.bill.paymentStatus!=="PAID" && !d.lineItems.some((l)=>l.serviceType==="CONSULTATION"&&l.sourceType==="OPD_VISIT")){
  ...
  const process=async()=>{
    if(!data?.bill)return;
    setLoading(true);setError("");setMessage("");
    try{await run("/api/billing",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({visitId:data.visit.id,action:"processBill"})});setMessage("Charges confirmed. Bill is ready for payment.");router.push(`/billing?visitId=${data.visit.id}`);}
    catch(e){setError(e instanceof Error?e.message:"Unable to process bill.");setLoading(false);}
  };

  return <main className="min-h-screen bg-[#fffaf0] p-4 text-[#172b3a] sm:p-6 lg:p-8">
    <div className="mx-auto max-w-7xl space-y-5">
      <header className="overflow-hidden rounded-[2rem] border border-[#d7a93d]/40 bg-gradient-to-br from-[#08483d] via-[#0d5b4c] to-[#176f5e] p-6 text-white shadow-xl sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-[10px] font-black uppercase tracking-[.28em] text-[#f5d47b]">SAMS · Patient Charges</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">Patient Charges</h1><p className="mt-2 max-w-2xl text-sm text-emerald-50">Build the patient bill from the selected OPD Visit — review, add, remove and discount charges before payment.</p></div>
          <div className="rounded-2xl border border-white/15 bg-white/10 px-5 py-4 text-xs"><p className="font-black uppercase tracking-widest text-[#f5d47b]">Workflow</p><p className="mt-1 font-bold">Patient → Visit → Charges → Review → Payment</p></div>
        </div>
      </header>

      <section className="rounded-[1.5rem] border border-[#eadfca] bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[9px] font-black uppercase tracking-widest text-[#0d5b4c]">Step 01</p><h2 className="text-xl font-black text-[#0b2748]">Find Patient & Select OPD Visit</h2></div><span className="rounded-full bg-[#edf5ee] px-3 py-1.5 text-[10px] font-black text-[#0d5b4c]">Visit-based billing</span></div>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row"><input value={patientId} onChange={e=>setPatientId(e.target.value)} onKeyDown={e=>e.key==="Enter"&&void findPatient()} placeholder="Enter Patient ID" className="flex-1 rounded-xl border border-[#d8dfe3] bg-[#fbfcfb] px-4 py-3 outline-none focus:border-[#0d5b4c]"/><button onClick={()=>void findPatient()} disabled={loading} className="rounded-xl bg-[#0d5b4c] px-6 py-3 font-black text-white shadow-sm disabled:opacity-50">{loading?"Loading…":"Find Patient"}</button></div>
        {visits.length>0&&<div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{visits.map(v=><button key={v.id} onClick={()=>void load(v.id)} className={`rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:border-[#0d5b4c] hover:shadow-md ${data?.visit.id===v.id?"border-[#0d5b4c] bg-[#edf5ee]":"border-[#eadfca] bg-white"}`}><div className="flex items-center justify-between"><b className="text-[#0b2748]">OPD Visit #{v.id}</b><span className="rounded-full bg-[#fff7dd] px-2 py-1 text-[9px] font-black text-[#8a6713]">Token #{v.tokenNumber}</span></div><p className="mt-2 text-xs text-slate-500">{v.visitType} · {v.department||"General"}</p><p className="mt-1 text-[10px] font-bold text-slate-400">{new Date(v.createdAt).toLocaleString()}</p></button>)}</div>}
      </section>

      {data&&<>
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Info title="Patient" value={data.patient.name}/><Info title="Patient ID" value={data.patient.patientId}/><Info title="OPD Visit" value={`#${data.visit.id} · Token #${data.visit.tokenNumber}`}/><Info title="Bill" value={data.bill?.billNumber||"Not created"}/></section>

        <section className="rounded-[1.5rem] border border-[#eadfca] bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-[#eadfca] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[9px] font-black uppercase tracking-widest text-[#0d5b4c]">Step 02</p><h2 className="text-xl font-black text-[#0b2748]">Review Patient Charges</h2><p className="mt-1 text-xs text-slate-500">Consultation fee is added automatically. Investigation charges appear only when they belong to this Visit bill.</p></div><span className={`rounded-full px-3 py-1.5 text-[10px] font-black ${data.bill?.paymentStatus==="PAID"?"bg-emerald-50 text-emerald-700":"bg-amber-50 text-amber-700"}`}>{data.bill?.paymentStatus||"DRAFT"}</span></div>
          <div className="p-5">
