...
const navItems = [
  ...
  ["Slip Print", "/patients/profile/1/opd-slip"],
] as const;
      ...
      <header className="sticky top-0 z-50 border-b border-[#d6a443]/30 bg-[#061525]/95 px-4 py-3 backdrop-blur sm:px-6 lg:px-10">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-4">
          <Link href="/" className="flex min-w-0 items-center gap-3">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-[#d6a443]/50 bg-white">
              <Image src="/serawat-logo.png" alt="SAMS logo" width={42} height={42} className="h-9 w-9 object-contain" />
            </span>
            <span className="min-w-0">
              <span className="block text-lg font-black tracking-wide text-[#d6a443]">SAMS</span>
              <span className="block max-w-[390px] truncate text-[10px] font-bold uppercase tracking-[0.16em] text-slate-200 sm:text-xs">SERAWAT ADVANCED MULTISPECIALITY JOINT &amp; SPINE CENTRE</span>
            </span>
          </Link>
          <nav className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-2">
            {navItems.map(([label, href]) => (
              <Link key={label} href={href} className={`rounded-xl px-3 py-2 text-xs font-black transition sm:px-4 ${label === "Doctor Desk" ? "bg-[#d6a443] text-[#071525]" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}>
                {label}
              </Link>
            ))}
          </nav>
