...
export default function DoctorDeskPage() {
  ...
  useEffect(() => {
    ...
    try { stored = JSON.parse(localStorage.getItem(DOCTOR_DESK_SELECTION_KEY) || "{}"); } catch { /* ignore */ }
  ...
  return (
    <main className="min-h-screen bg-[#050c16] text-slate-100">
      <header className="sticky top-0 z-50 border-b border-[#d6a443]/30 bg-[#061525]/95 px-4 py-3 backdrop-blur sm:px-6 lg:px-10">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-4">
          <Link href="/" className="flex min-w-0 items-center gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-[#d6a443]/50 bg-white"><Image src="/serawat-logo.png" alt="SAMS logo" width={42} height={42} className="h-9 w-9 object-contain" /></span><span className="min-w-0"><span className="block text-lg font-black tracking-wide text-[#d6a443]">SAMS</span><span className="block max-w-[390px] truncate text-[10px] font-bold uppercase tracking-[0.16em] text-slate-200 sm:text-xs">SERAWAT ADVANCED MULTISPECIALITY JOINT &amp; SPINE CENTRE</span></span></Link>
          <nav className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-2">{navItems.map(([label, href]) => <Link key={label} href={href} className={`rounded-xl px-3 py-2 text-xs font-black transition sm:px-4 ${label === "Doctor Desk" ? "bg-[#d6a443] text-[#071525]" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}>{label}</Link>)}</nav>
        </div>
      </header>

      <div className="px-3 py-4 sm:px-6 lg:px-8 lg:py-6">
        <section className="relative mx-auto max-w-[1500px] overflow-hidden rounded-[2.25rem] border border-[#d6a443]/45 bg-[linear-gradient(135deg,#061525,#0c263d_55%,#050e18)] px-5 py-7 shadow-[0_30px_90px_rgba(0,0,0,.48)] sm:px-10 sm:py-9"><div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-[#082b61] via-[#d6a443] to-[#f4d58c]" /><div className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-[0.04]"><Image src="/serawat-logo.png" alt="" width={600} height={600} className="h-[500px] w-[500px] object-contain" /></div><div className="relative z-10 flex flex-wrap items-center justify-between gap-5"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#d6a443]">SAMS · Clinical Operations</p><h1 className="mt-2 text-3xl font-black tracking-tight text-white sm:text-5xl">Doctor Desk</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Select your department and doctor, monitor today’s OPD, call the next patient and enter consultation.</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.05] px-5 py-4 text-right"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-[#d6a443]">Active Queue</p><p className="mt-1 text-3xl font-black text-white">{activeVisits.length}</p></div></div></section>
        ...
        <section className="mx-auto mt-6 max-w-[1500px] rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(9,27,45,.94),rgba(4,14,25,.97))] p-5 shadow-[0_22px_70px_rgba(0,0,0,.32)] sm:p-7">
          <div className="grid gap-4 lg:grid-cols-2"><label className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><span className="text-[9px] font-black uppercase tracking-[0.18em] text-[#d6a443]">Select Your Department</span><select value={departmentId} onChange={e => { setDepartmentId(e.target.value); setDoctorId(""); }} disabled={loading} className="mt-2 w-full rounded-xl border border-white/10 bg-[#071525] px-4 py-3 text-sm font-bold text-white outline-none focus:border-[#d6a443]"><option value="">Choose department</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}{d.code ? ` · ${d.code}` : ""}</option>)}</select></label><label className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><span className="text-[9px] font-black uppercase tracking-[0.18em] text-[#d6a443]">Doctor</span><select value={doctorId} onChange={e => setDoctorId(e.target.value)} disabled={!departmentId || !doctors.length} className="mt-2 w-full rounded-xl border border-white/10 bg-[#071525] px-4 py-3 text-sm font-bold text-white outline-none focus:border-[#d6a443] disabled:opacity-50"><option value="">All consultants</option>{doctors.map(d => <option key={d.id} value={d.id}>{doctorName(d)}</option>)}</select></label></div>
