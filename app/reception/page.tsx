        ...
        <div className="relative overflow-hidden px-3 py-4 sm:px-6 lg:px-8 lg:py-6">
          ...
          <section className="relative mx-auto max-w-[1500px] overflow-hidden rounded-[2.25rem] border border-[#d6a443]/45 bg-[linear-gradient(135deg,rgba(5,18,31,0.94),rgba(12,38,61,0.90)_52%,rgba(5,14,24,0.95))] px-5 py-8 text-center shadow-[0_30px_90px_rgba(0,0,0,0.48)] sm:px-10 sm:py-10 lg:px-16 lg:py-11">
            ...
            <div className="relative z-10 mx-auto flex max-w-6xl flex-col items-center">
              ...
              <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-slate-200 drop-shadow-[0_2px_8px_rgba(0,0,0,0.65)] sm:text-base lg:text-lg">Welcome to the SAMS reception desk — where every patient journey begins with a warm welcome, efficient registration and coordinated care.</p>
              <div className="mt-6 flex flex-wrap items-center justify-center gap-2.5 text-[9px] font-black uppercase tracking-[0.16em] text-[#f4d58c] sm:text-[10px]">{[["👋","Welcome"],["📝","Register"],["✓","Verify"],["🏥","OPD"],["🩺","Consultation"],["❤️","Care"],["💳","Billing"],["📅","Follow-up"]].map(([icon,label],index)=><div key={label} className="flex items-center gap-2 rounded-full border border-white/10 bg-[#071525]/65 px-3 py-2 shadow-sm backdrop-blur-sm"><span className="text-sm">{icon}</span><span>{label}</span>{index<7&&<span className="ml-1 text-[#d6a443]">→</span>}</div>)}</div>
            </div>
          </section>
