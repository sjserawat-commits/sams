            ...
            <table className="w-full min-w-[1250px] text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.map((row) => <tr key={row.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3"><p className="font-black text-[#082b61]">{row.name}</p><p className="mt-0.5 font-mono text-[9px] text-slate-400">{row.code} {row.shortName ? `· ${row.shortName}` : ""}</p></td>
                  ...
                  <td className="px-4 py-3 text-right"><button onClick={() => setEditing(row)} className="rounded-lg border border-[#0b63ce]/20 bg-[#0b63ce]/5 px-3 py-1.5 text-[10px] font-black text-[#0b63ce]">Edit Master</button></td>
                </tr>)}
...
function EditInvestigation({ investigation, onSaved, onCancel }: { investigation: Investigation; onSaved: () => void; onCancel: () => void }) {
  ...
  return <section className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4 sm:p-8">
    <div className="mx-auto max-w-5xl rounded-[2rem] bg-white p-6 shadow-2xl sm:p-8">
      <div className="flex items-start justify-between gap-4"><div><p className="text-[9px] font-black uppercase tracking-widest text-[#0b63ce]">Admin master data</p><h2 className="mt-1 text-2xl font-black text-[#082b61]">{investigation.name}</h2><p className="mt-1 text-xs font-semibold text-slate-400">Code: {investigation.code}</p></div><button onClick={onCancel} className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-black">Close</button></div>
      ...
      <div className="mt-6 flex justify-end gap-3"><button onClick={onCancel} className="rounded-xl border border-slate-200 px-5 py-3 text-xs font-black">Cancel</button><button disabled={saving} onClick={save} className="rounded-xl bg-[#082b61] px-6 py-3 text-xs font-black text-white disabled:opacity-50">{saving ? "Saving…" : "Save Master Data"}</button></div>
      <p className="mt-4 text-[10px] leading-5 text-slate-400">Reference intervals are method-, age- and sex-dependent. The configured values are master defaults and should be aligned with the performing laboratory's validated report intervals.</p>
    </div>
