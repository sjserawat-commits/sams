...
function formatDob(value?: string) {
  ...
  if (!year || !month || !day) return "Date of birth not recorded";
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="relative min-h-[calc(100vh-64px)] overflow-hidden">
          ...
          <div className="relative mx-auto max-w-[1500px] px-4 py-7 sm:px-7 lg:px-10 lg:py-9">
            <header className="mb-7"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#35705a]">SAMS · Clinical Operations</p><h1 className="mt-1 text-3xl font-black tracking-tight text-[#12382a] sm:text-5xl">Patient Registry</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">A spacious directory for finding, verifying and continuing an existing patient&apos;s care.</p></div></header>
            <section className="rounded-[1.5rem] border border-white/80 bg-white/90 p-5 shadow-[0_18px_50px_rgba(35,70,55,0.10)] backdrop-blur-md sm:p-7"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#35705a]">Returning Patient Workflow</p><h2 className="mt-1 text-xl font-black text-[#173f30] sm:text-2xl">Find an existing patient</h2><p className="mt-1 text-sm text-slate-500">Search by Patient ID, name, mobile number, date of birth or gender.</p></div><div className="flex w-full max-w-2xl items-center rounded-xl border border-slate-200 bg-white p-1.5 shadow-sm"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#edf5f0] text-lg text-[#35705a]">⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search patient ID, name, mobile or DOB" className="h-10 w-full bg-transparent px-3 text-sm font-medium text-slate-800 outline-none placeholder:text-slate-400" aria-label="Search patients" />{query && <button type="button" onClick={() => setQuery("")} className="mr-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-500 hover:bg-slate-100">Clear</button>}</div></div></section>
            <section className="mt-6 overflow-hidden rounded-[1.5rem] border border-white/80 bg-white/95 shadow-[0_18px_55px_rgba(35,70,55,0.12)]"><div className="flex flex-col gap-3 border-b border-slate-100 bg-white/90 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#35705a]">Patient Directory</p><h2 className="mt-1 text-xl font-black text-[#173f30] sm:text-2xl">{loading ? "Loading records…" : `${filtered.length} matching record${filtered.length === 1 ? "" : "s"}`}</h2></div><span className="w-fit rounded-full border border-[#b88b2d]/30 bg-[#fff9ec] px-3 py-1.5 text-[9px] font-black uppercase tracking-wider text-[#87671e]">Clinical Records</span></div>{loading ? <div className="px-6 py-16 text-center text-sm font-semibold text-slate-500">Loading patient records…</div> : filtered.length === 0 ? <div className="px-6 py-16 text-center"><p className="text-lg font-black text-[#173f30]">No matching patient found</p><p className="mt-2 text-sm text-slate-500">Try another search or register a new patient.</p><Link href="/patients/new" className="mt-5 inline-flex rounded-xl bg-[#173f30] px-5 py-3 text-sm font-bold text-white">Register New Patient</Link></div> : <div className="overflow-x-auto"><div className="min-w-[780px]"><div className="grid grid-cols-[1.5fr_170px_170px_120px] bg-[#173f30] px-6 py-3 text-[9px] font-black uppercase tracking-[0.18em] text-[#f2d58b]"><span>Patient</span><span>Patient ID</span><span>Contact</span><span className="text-right">Action</span></div>{filtered.slice(0, 100).map((p, index) => <div key={p.id} className={`grid grid-cols-[1.5fr_170px_170px_120px] items-center px-6 py-4 transition hover:bg-[#f1f7f3] ${index < Math.min(filtered.length, 100) - 1 ? "border-b border-slate-100" : ""}`}><div className="flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#b88b2d]/35 bg-[#fff8e8] text-sm font-black text-[#87671e]">{(p.firstName?.[0] || "P").toUpperCase()}</div><div><p className="font-bold text-[#173f30]">{[p.firstName, p.lastName].filter(Boolean).join(" ") || "Unnamed patient"}</p><p className="mt-0.5 text-[10px] font-medium text-slate-400">{formatDob(p.dateOfBirth)}</p></div></div><span className="text-sm font-semibold text-slate-600">{p.patientId ?? p.id}</span><span className="text-sm text-slate-500">{p.phone ?? "Not provided"}</span><div className="text-right"><Link href={`/patients/profile/${p.id}`} className="inline-flex rounded-lg border border-[#35705a]/20 bg-[#edf5f0] px-3 py-2 text-xs font-black text-[#25634b] transition hover:bg-[#173f30] hover:text-white">Open Profile →</Link></div></div>)}</div></div>}{filtered.length > 100 && <p className="border-t border-slate-100 px-6 py-3 text-center text-xs font-semibold text-slate-400">Showing first 100 results. Refine your search to find a specific record.</p>}</section>
          </div>
        </div>
      </div>
    </main>
