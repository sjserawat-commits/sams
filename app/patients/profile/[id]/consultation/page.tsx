...
const foodOptions = ["Before food", "After food", "With food", "Empty stomach", "At bedtime", "Any time"];
const durationOptions = Array.from({ length: 30 }, (_, i) => String(i + 1));
...
export default function ConsultationPage() {
  ...
  async function reload(reopen = false) {
    if (!visitId) return;
    try {
      if (reopen) {
        const reopenResponse = await fetch(`/api/opd/${visitId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status: "IN_CONSULTATION", reopen: true }),
        });
        const reopenData = await reopenResponse.json();
        if (!reopenResponse.ok) throw new Error(reopenData?.error || "Unable to reopen consultation.");
        setFinalized(false);
        setVisit(reopenData);
        setMessage("Saved consultation reopened. You can continue editing.");
      }
      ...
      if (encounterResponse.ok) {
        ...
        if (c) {
          ...
          } else if (Array.isArray(c.prescriptions) && c.prescriptions.length) {
            setMedicines(c.prescriptions.map((p: any, index: number) => ({ id: Number(p.id) || Date.now() + index, name: String(p.medicineName || ""), dose: String(p.dose || ""), frequency: String(p.frequency || ""), food: "", duration: String(p.duration || "") })));
          }
    ...
    <div className="mx-auto max-w-[1600px] px-4 py-5 sm:px-7 lg:px-9"><section className="print-sheet overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-xl">
      ...
      <div className="grid lg:grid-cols-[230px_minmax(0,1fr)]">
        ...
        <section className="p-5 sm:p-8"><div className="grid gap-5 lg:grid-cols-2"><div className="rounded-2xl border p-4"><p className="mb-2 text-[10px] font-black uppercase tracking-[.18em] text-[#082b61]">Chief Complaint</p><textarea className="w-full rounded-lg border p-2 text-xs" value={complaint} onChange={e => setComplaint(e.target.value)} rows={5}/></div><div className="rounded-2xl border p-4"><p className="mb-2 text-[10px] font-black uppercase tracking-[.18em] text-[#082b61]">Clinical Diagnosis</p><textarea className="w-full rounded-lg border p-2 text-xs" value={diagnosis} onChange={e => setDiagnosis(e.target.value)} rows={5}/></div></div>
          <div className="mt-5 rounded-2xl border p-4 no-print"><div className="flex items-center justify-between"><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#082b61]">Medicines / Prescription</p><button type="button" onClick={() => setMedicines(v => [...v, { id: Date.now(), name: "", dose: "", frequency: "", food: "", duration: "" }])} className="rounded-lg bg-[#082b61] px-3 py-2 text-[10px] font-black text-white">+ Add Medicine</button></div>{medicines.map((m, i) => <div key={m.id} className="mt-3 grid gap-2 rounded-xl bg-slate-50 p-2 sm:grid-cols-5"><input className="rounded-lg border px-2 py-2 text-xs" value={m.name} onChange={e => setMedicines(v => v.map(x => x.id === m.id ? { ...x, name: e.target.value } : x))} placeholder={`Medicine ${i + 1}`}/><input className="rounded-lg border px-2 py-2 text-xs" value={m.dose} onChange={e => setMedicines(v => v.map(x => x.id === m.id ? { ...x, dose: e.target.value } : x))} placeholder="Dose"/><select className="rounded-lg border bg-white px-2 py-2 text-xs" value={m.frequency} onChange={e => setMedicines(v => v.map(x => x.id === m.id ? { ...x, frequency: e.target.value } : x))}><option value="">Frequency</option>{frequencyOptions.map(x => <option key={x} value={x}>{x}</option>)}</select><select className="rounded-lg border bg-white px-2 py-2 text-xs" value={m.food} onChange={e => setMedicines(v => v.map(x => x.id === m.id ? { ...x, food: e.target.value } : x))}><option value="">Food</option>{foodOptions.map(x => <option key={x} value={x}>{x}</option>)}</select><select className="rounded-lg border bg-white px-2 py-2 text-xs" value={m.duration} onChange={e => setMedicines(v => v.map(x => x.id === m.id ? { ...x, duration: e.target.value } : x))}><option value="">Duration</option>{durationOptions.map(x => <option key={x} value={x}>{x} day{x === "1" ? "" : "s"}</option>)}</select></div>)}</div>
          ...
          <div className="mt-6 flex flex-col gap-3 border-t pt-5 no-print sm:flex-row sm:items-end sm:justify-between"><div><span className="block text-[9px] font-black uppercase tracking-wider text-slate-400">Follow-up Date</span><input type="date" value={followUp} onChange={e => setFollowUp(e.target.value)} className="mt-2 rounded-lg border p-2 text-xs"/></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => reload(true)} disabled={!visitId || saving || finalizing} className="rounded-xl border border-[#0b63ce] bg-white px-4 py-3 text-xs font-black text-[#0b63ce]">↻ Reopen Saved Consultation</button><button type="button" onClick={() => save()} disabled={saving || finalized} className="rounded-xl bg-[#0b63ce] px-5 py-3 text-xs font-black text-white disabled:opacity-50">{saving ? "Saving…" : "Save Consultation · Keep Visit Open"}</button><button type="button" onClick={() => finalize(false)} disabled={finalizing || finalized} className="rounded-xl bg-[#082b61] px-5 py-3 text-xs font-black text-white disabled:opacity-50">{finalizing ? "Finalizing…" : "Finalize Consultation"}</button><button type="button" onClick={() => finalize(true)} disabled={finalizing || finalized} className="rounded-xl bg-[#0b63ce] px-5 py-3 text-xs font-black text-white disabled:opacity-50">Finalize &amp; Print</button>{finalized && <button type="button" onClick={() => router.push(`/patients/profile/${pid}/opd-slip/print?visitId=${visitId}`)} className="rounded-xl border border-[#082b61] bg-white px-5 py-3 text-xs font-black text-[#082b61]">Print Final Consultation</button>}</div></div>
