...
export async function GET(request: Request) {
  try {
    ...
    const orders = await prisma.investigationOrder.findMany({
      where: { ...(visitId > 0 ? { opdVisitId: visitId } : {}), ...(status && ALLOWED_STATUS.includes(status as InvestigationStatus) ? { status } : {}), ...(q ? { OR: [{ investigation: { contains: q } }, { opdVisit: { patient: { patientId: { contains: q } } } }, { opdVisit: { patient: { firstName: { contains: q } } } }, { opdVisit: { patient: { lastName: { contains: q } } } }] } : {}) },
      include: { master: { select: { code:true, category:true, specimen:true, unit:true, referenceRange:true, method:true, criticalValue:true } }, opdVisit: { select: { id:true, tokenNumber:true, visitType:true, patient:{select:{patientId:true,firstName:true,lastName:true,gender:true,dateOfBirth:true}} , billingRecords:{select:{id:true,paymentStatus:true,paidAmount:true,balanceAmount:true,netAmount:true},orderBy:{createdAt:"desc"},take:1} } },
      orderBy: { createdAt: "desc" },
    });
    return NextResponse.json(orders.map((order)=>{ const bill=order.opdVisit.billingRecords[0]??null; const paid=!bill||bill.balanceAmount<=0||bill.paymentStatus==="PAID"; const approved=["APPROVED_FOR_SAMPLING","ACCEPTED","SAMPLE_COLLECTED","PROCESSING","COMPLETED","VERIFIED","PUBLISHED"].includes(order.status); return {...order,billing:bill,workflow:{paymentStatus:bill?.paymentStatus??order.paymentStatus,outstandingAmount:bill?.balanceAmount??Math.max(0,order.netAmount),samplingEligible:paid||approved,paymentRequiredBeforeSampling:!paid&&!approved}};}));
  } catch { return NextResponse.json({error:"Unable to load investigation room."},{status:500}); }
}

export async function PATCH(request: Request) {
  try {
    const body=await request.json(); const id=Number(body?.id); const requestedStatus=String(body?.status??"").trim().toUpperCase();
    ...
    const bill=order.opdVisit.billingRecords[0]??null; const paid=!bill||bill.balanceAmount<=0||bill.paymentStatus==="PAID"; const samplingApproved=["APPROVED_FOR_SAMPLING","ACCEPTED"].includes(order.status);
    ...
    const now=new Date(); const data:any={status:requestedStatus};
