    ...
    const visits = await prisma.oPDVisit.findMany({
      ...
      include: {
        ...
        investigationOrders: {
          ...
          select: { id: true, investigation: true, status: true, paymentStatus: true },
        },
      },
    ...
    return NextResponse.json(
      visits.map((visit) => ({
        ...
        consultation: visit.clinicalEncounter
          ? { id: visit.clinicalEncounter.id, hasNotes: Boolean(visit.clinicalEncounter.clinicalNotes?.trim()) }
          : null,
