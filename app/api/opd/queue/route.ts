...
export async function GET(request: Request) {
  try {
    ...
    if (!departmentId) return NextResponse.json({ error: "Department desk is required." }, { status: 400 });
    ...
    const visits = await prisma.oPDVisit.findMany({
      where: { departmentId, createdAt: { gte: start, lt: end }, status: { in: ["WAITING", "IN_CONSULTATION", "COMPLETED"] } },
