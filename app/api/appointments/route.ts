...
export async function GET() {
  try {
    const appointments = await prisma.appointment.findMany({
      orderBy: [{ appointmentDate: "asc" }, { appointmentTime: "asc" }, { id: "asc" }],
    });
...
/**
 * Checked-in appointment -> OPD Registration.
 * For a NEW patient Aadhaar is captured here, not during appointment booking.
 * The created OPD visit keeps the appointmentId relation so Queue Manager can
 * identify appointment patients and apply slot-aware priority.
 */
export async function PATCH(request: Request) {
  try {
    ...
    if (body.action === "REGISTER_OPD") {
      if (appointment.opdVisitId) {
        const existing = await prisma.oPDVisit.findUnique({ where: { id: appointment.opdVisitId } });
        if (existing) return NextResponse.json({ appointment, visit: existing, alreadyRegistered: true });
      }
      if (appointment.status !== "CHECKED_IN") {
        return NextResponse.json({ error: "Patient must be checked-in before OPD registration." }, { status: 400 });
      }
      if (!appointment.departmentId || !appointment.doctorId) {
        return NextResponse.json({ error: "Department and consultant are required for OPD registration." }, { status: 400 });
      }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (appointment.patientType === "NEW") {
          ...
          if (aadhaar.length !== 12) throw new Error("Aadhaar number is required for a new patient and must contain exactly 12 digits.");
          const duplicate = await tx.patient.findFirst({ where: { aadhaarNumber: aadhaar }, select: { id: true, patientId: true } });
          if (duplicate) throw new Error(`Aadhaar already belongs to patient ${duplicate.patientId}. Use Existing Patient.`);

          const parts = appointment.patientName.trim().split(/\s+/);
          const firstName = parts.shift() || "Patient";
          const lastName = parts.join(" ") || "-";
          const lastPatient = await tx.patient.findFirst({ orderBy: { id: "desc" }, select: { patientId: true } });
          const lastNumber = Number(lastPatient?.patientId?.match(/(\d+)$/)?.[1] || 0);
          const newPatient = await tx.patient.create({
            data: {
              patientId: `SAMS-${String(lastNumber + 1).padStart(4, "0")}`,
              firstName,
              lastName,
        ...
        if (!patientId) throw new Error("Existing patient appointment must be linked to a patient/HID before OPD registration.");

        const lastVisit = await tx.oPDVisit.findFirst({
          where: { departmentId: appointment.departmentId, createdAt: { gte: start, lt: end } },
          orderBy: [{ tokenNumber: "desc" }, { id: "desc" }],
        ...
        const updatedAppointment = await tx.appointment.update({
          ...
          data: { patientId, opdVisitId: visit.id, status: "OPD_REGISTERED" },
        });
        return { visit, appointment: updatedAppointment, patientId };
      });
    ...
    const updated = await prisma.appointment.update({
      ...
      data: {
        ...
        ...(body.opdVisitId ? { opdVisitId: Number(body.opdVisitId) } : {}),
      },
    });
    return NextResponse.json(updated);
  } catch (error) {
    console.error("Appointment update failed", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to update appointment." }, { status: 500 });
  }
}
