...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO "SamsUser" ("username","displayName","passwordHash","role","createdAt","updatedAt") VALUES (?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`,
      username,
      displayName,
      passwordHash,
      "SUPER_ADMIN",
    ...
    return NextResponse.json(
      { ok: true, message: "Administrator created. Sign in to continue." },
      { status: 201 },
    );
  } catch (error) {
    console.error("SAMS first-run setup failed:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to create administrator." },
      { status: 500 },
    );
  }
}
