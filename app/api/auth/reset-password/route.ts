...
export async function POST(req: NextRequest) {
  try {
    ...
    // Dedicated recovery key is preferred. During local/development setup,
    // the first-run setup key remains a valid recovery key so an administrator
    // who forgot the password is not locked out of the development instance.
    const expectedKey = process.env.SAMS_PASSWORD_RESET_KEY || process.env.SAMS_SETUP_KEY;
    if (!expectedKey || recoveryKey !== expectedKey) {
      return NextResponse.json({ error: "Invalid recovery key." }, { status: 403 });
    }

    const users = await prisma.$queryRawUnsafe<Array<{ id: number; active: number }>>(
