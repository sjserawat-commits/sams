...
export async function GET(request:Request){
 try{
  ...
  const billedOutstanding=round(bills.reduce((s,b)=>s+b.balanceAmount,0));
  const unbilledOutstanding=round(lines.filter(l=>l.billingRecordId===null&&l.billStatus==="UNBILLED").reduce((s,l)=>s+l.amount,0));
  const outstanding=round(billedOutstanding+unbilledOutstanding);
  return NextResponse.json({patient:{id:p.id,patientId:p.patientId,name:`${p.firstName} ${p.lastName}`.trim(),age:age(p.dateOfBirth),gender:p.gender,phone:p.phone},visits:p.opdVisits.map(v=>({id:v.id,tokenNumber:v.tokenNumber,visitType:v.visitType,status:v.status,department:v.departmentMaster?.name||null,doctor:v.doctor?.name||null,createdAt:v.createdAt})),lines,bills,outstanding});
