  ...
  return (
    <main className="min-h-screen bg-[#f8f4ea] grid place-items-center p-5">
      <form onSubmit={submit} className="w-full max-w-md rounded-[2rem] bg-white p-8 shadow-xl border border-[#eadfca]">
        <p className="text-[10px] font-black tracking-[.3em] text-[#0d5b4c]">SAMS · PASSWORD</p>
        <h1 className="mt-2 text-3xl font-black text-[#0b2748]">Set new password</h1>
        <p className="mt-1 text-sm text-slate-500">Enter your username and choose a new password.</p>
        <div className="mt-7 space-y-4">
          ...
          <button disabled={busy} className="w-full rounded-xl bg-[#0d5b4c] p-3 font-black text-white disabled:opacity-50">{busy ? "Updating password…" : "Set new password"}</button>
          <button type="button" onClick={() => router.replace("/login")} className="w-full rounded-xl border border-slate-200 p-3 font-bold text-slate-700">Back to sign in</button>
        </div>
      </form>
    </main>
  );
