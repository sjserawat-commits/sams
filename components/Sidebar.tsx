...
function NavItem({ href, label, icon }: { href: string; label: string; icon: string }) {
  return (
    <Link
      href={href}
      className="group flex min-w-0 w-full items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 text-sm font-bold text-slate-300 transition-all hover:border-[#d6a443]/30 hover:bg-white/5 hover:text-[#f2d38b]"
    >
      ...
      <span className="block min-w-0 flex-1 whitespace-nowrap text-left leading-5">{label}</span>
    </Link>
  );
}
...
export default function Sidebar() {
  return (
    <aside className="sticky top-0 flex h-screen w-full min-w-0 flex-col overflow-y-auto border-r border-[#d6a443]/20 bg-[linear-gradient(180deg,#061525_0%,#0a1d31_48%,#071525_100%)] px-4 py-5 shadow-[12px_0_40px_rgba(0,0,0,0.28)]">
      <div className="relative mb-8 min-h-[112px] shrink-0 overflow-hidden rounded-[1.25rem] border border-[#d6a443]/35 bg-[linear-gradient(135deg,#0b3158,#071b30)] p-4 text-white shadow-[0_16px_35px_rgba(0,0,0,0.25)]">
        ...
        <Link href="/" className="relative flex min-w-0 items-start gap-3">
          ...
          <span className="min-w-0 flex-1 pt-0.5">
            <span className="block text-lg font-black leading-5 tracking-tight text-[#f2d38b]">SAMS</span>
            <span className="mt-1 block text-[8px] font-bold uppercase leading-3 tracking-[0.11em] text-slate-300">Serawat Advanced Multispeciality Joint & Spine Centre</span>
          </span>
        </Link>
      </div>

      <div className="mb-2 px-2 text-[9px] font-black uppercase tracking-[0.22em] text-[#d6a443]">Quick Access</div>
      <nav className="mb-5 min-w-0 space-y-1">
