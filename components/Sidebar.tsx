...
function NavItem({ href, label, icon }: { href: string; label: string; icon: string }) {
  return (
    <Link
      href={href}
      className="group flex w-full items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 text-sm font-bold transition-all hover:border-[#d6a443]/30 hover:bg-white/5"
    >
      ...
      <strong
        className="block flex-1 whitespace-nowrap text-left text-sm font-bold leading-5"
        style={{ display: "block", visibility: "visible", opacity: 1, color: "#cbd5e1" }}
      >
        {label}
      </strong>
    </Link>
      ...
      <div className="relative mb-8 min-h-[112px] shrink-0 overflow-hidden rounded-[1.25rem] border border-[#d6a443]/35 bg-[linear-gradient(135deg,#0b3158,#071b30)] p-4 text-white shadow-[0_16px_35px_rgba(0,0,0,0.25)]">
        ...
        <Link href="/" className="relative flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            ...
            <div className="mt-1 block text-[8px] font-bold uppercase leading-3 tracking-[0.11em] text-slate-300">Serawat Advanced Multispeciality Joint & Spine Centre</div>
          </div>
        </Link>
      </div>
