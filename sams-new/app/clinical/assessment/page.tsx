...
type AssessmentData = { standard?: string; sections: Record<string, Record<string, string>> };
...
export default function AssessmentPage() {
  ...
  const [data, setData] = useState<AssessmentData>({ sections: {}, standard });
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");

  function updateField(key: string, value: string) {
    setData((current) => ({ ...current, standard: current.standard || standard, sections: { ...current.sections, [activeSection]: { ...(current.sections[activeSection] || {}), [key]: value } } }));
  }

  async function saveAssessment() {
    setError(""); setMessage("");
    if (!encounterId) { setError("Open this assessment from a clinical encounter so it can be saved to that encounter."); return; }
    setSaving(true);
    try {
      const response = await fetch(`/api/patients/encounters/${encounterId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ assessmentData: { ...data, standard: data.standard || standard } }) });
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-5 sm:p-7">
      <div className="mx-auto max-w-7xl">
        <section className="overflow-hidden rounded-3xl bg-[#082b61] shadow-lg"><div className="px-6 py-8 text-white sm:px-9 sm:py-10"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-blue-100">Clinical Workspace</span><h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">Assessment & PM&R</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100 sm:text-base">Structured assessment for clinical findings, function, rehabilitation needs and management planning.</p>{encounterId && <p className="mt-2 text-xs font-semibold text-blue-200">Current encounter #{encounterId}{standard ? ` · ${standard}` : ""}</p>}</div><div className="flex flex-wrap gap-3"><Link href={patientId && encounterId ? `/clinical/assessment/standard?patientId=${patientId}&encounterId=${encounterId}` : "/clinical/assessment/standard"} className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-[#082b61] shadow-sm hover:bg-blue-50">Standard Assessments</Link><Link href="/clinical" className="rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 text-sm font-semibold hover:bg-white/20">← Clinical</Link></div></div></div></section>
        {!encounterId && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-semibold text-amber-800">Open this page from a clinical encounter to save the assessment against that encounter.</div>}
        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[250px_1fr]"><aside className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><p className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-slate-400">Assessment Workflow</p><div className="space-y-1">{sections.map((section) => <button key={section} onClick={() => setActiveSection(section)} className={`w-full rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${activeSection === section ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50"}`}>{section}</button>)}</div></aside>
