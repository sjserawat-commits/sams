...
export default async function VisitDetailPage({ params }: { params: Promise<{ id: string; encounterId: string }> }) {
  ...
  if (encounter.assessmentData) {
    try { assessmentSummary = JSON.parse(encounter.assessmentData) as { standard?: string }; } catch { assessmentSummary = {}; }
  }
    ...
    <main className="min-h-screen bg-[#f5f8fc] text-slate-900">
      ...
      <div className="mx-auto max-w-[1200px] px-5 py-7 sm:px-8 sm:py-10">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#0b63ce]">SAMS · Clinical Visit</p><p className="mt-1 text-sm text-slate-500">{patient.firstName} {patient.lastName} · {patient.patientId}</p></div>
