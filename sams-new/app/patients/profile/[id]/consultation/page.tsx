    ...
    <>
        ...
        <div className="mx-auto max-w-[1600px] px-4 py-5 sm:px-7 lg:px-9">
          <section className="consultation-paper overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-xl">
            <section className="border-b border-slate-200 bg-[#fbfaf7] px-5 py-5 sm:px-8">
              ...
              <div className="mt-4 grid gap-x-6 gap-y-3 text-xs sm:grid-cols-2 lg:grid-cols-4"><div><span className="label">Patient ID</span><strong>{patientIdDisplay}</strong></div><div><span className="label">Age / Sex</span><strong>{ageFromDob(patientDob)} / {String(patient?.gender || "—")}</strong></div><div><span className="label">Mobile</span><strong>{patientPhone}</strong></div><div><span className="label">DOB</span><strong>{patientDob ? new Date(patientDob).toLocaleDateString("en-IN") : "—"}</strong></div><div><span className="label">Department</span><strong>{department}</strong></div><div><span className="label">Consultant</span><strong>{doctor}</strong></div><div><span className="label">Visit No.</span><strong>{visit?.id || "—"}</strong></div><div><span className="label">Date</span><strong>{new Date().toLocaleDateString("en-IN")}</strong></div></div>
            </section>
      ...
      <section className="print-document" aria-hidden="true">
        ...
        <div className="print-patient">
          <div><span>Patient</span><strong>{patientName}</strong></div><div><span>Patient ID</span><strong>{patientIdDisplay}</strong></div><div><span>Age / Sex</span><strong>{ageFromDob(patientDob)} / {String(patient?.gender || "—")}</strong></div><div><span>Mobile</span><strong>{patientPhone}</strong></div><div><span>DOB</span><strong>{patientDob ? new Date(patientDob).toLocaleDateString("en-IN") : "—"}</strong></div><div><span>Visit No.</span><strong>{visit?.id || "—"}</strong></div><div><span>Department</span><strong>{department}</strong></div><div><span>Consultant</span><strong>{doctor}</strong></div></div>
