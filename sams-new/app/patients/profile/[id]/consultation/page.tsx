...
export default function ConsultationPage() {
  ...
  const consultationDate = consultationDateTime.toLocaleDateString("en-IN");
  const consultationTime = consultationDateTime.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" });
    ...
    <>
        ...
        <header className="no-print sticky top-0 z-40 border-b border-[#d7c08a]/30 bg-[#071f46] text-white shadow-lg">
          <div className="mx-auto flex min-h-[92px] max-w-[1600px] items-center justify-between gap-5 px-5 py-4 sm:px-8">
            <div className="flex min-w-0 items-center gap-4">
              ...
              <div className="min-w-0 border-l border-white/15 pl-4"><p className="text-[9px] font-black uppercase tracking-[0.25em] text-[#f1d27a]">SAMS · Clinical Record</p><h1 className="mt-1 truncate text-xl font-semibold tracking-tight sm:text-2xl" style={{ fontFamily: "'Cormorant Garamond','Baskerville','Times New Roman',serif" }}>Serawat Advanced Musculoskeletal, Joint &amp; Spine Centre</h1><p className="mt-0.5 text-[11px] font-medium text-blue-100">Consultation / Clinical Record</p></div>
            </div>
        ...
        <div className="mx-auto max-w-[1600px] px-4 py-5 sm:px-7 lg:px-9">
          <section className="consultation-paper overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-xl">
            <section className="border-b border-slate-200 bg-[#fbfaf7] px-5 py-5 sm:px-8">
              ...
              <div className="mt-4 grid gap-x-6 gap-y-3 text-xs sm:grid-cols-2 lg:grid-cols-4"><div><span className="label">Patient ID</span><strong>{patientIdDisplay}</strong></div><div><span className="label">Age / Sex</span><strong>{ageFromDob(patientDob)} / {String(patient?.gender || "—")}</strong></div><div><span className="label">Mobile</span><strong>{patientPhone}</strong></div><div><span className="label">DOB</span><strong>{patientDob ? new Date(patientDob).toLocaleDateString("en-IN") : "—"}</strong></div><div><span className="label">Department</span><strong>{department}</strong></div><div><span className="label">Consultant</span><strong>{doctor}</strong></div><div><span className="label">Visit No.</span><strong>{visit?.id || "—"}</strong></div><div><span className="label">Date &amp; Time</span><strong>{consultationDate} · {consultationTime}</strong></div></div>
            </section>
            ...
            <div className="grid lg:grid-cols-[220px_minmax(0,1fr)]">
              ...
              <section className="min-w-0 p-5 sm:p-8">
                ...
                {error && <div className="no-print mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</div>}{saved && <div className="no-print mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm font-bold text-emerald-700">Consultation saved successfully. Use Print A4 for the final single-page record.</div>}
                <div className="mt-6 flex items-end justify-between gap-6 border-t border-slate-200 pt-5"><div><label className="block text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Follow-up Date</label><input type="date" value={followUpDate} onChange={e => setFollowUpDate(e.target.value)} className="no-print mt-2 rounded-lg border border-slate-200 p-2 text-xs" /></div><div className="w-56 text-center"><div className="h-8 border-b border-slate-400"/><p className="mt-1 text-xs font-black text-[#082b61]">Consultant</p><p className="text-[10px] font-semibold text-slate-500">{doctor}</p><p className="no-print mt-1 text-[9px] text-slate-400">Signature</p></div></div>
      ...
      <style jsx global>{`
        ...
        @media print{
          ...
          html,body{margin:0!important;padding:0!important;width:210mm!important;height:297mm!important;max-height:297mm!important;overflow:hidden!important;background:#fff!important}
          body{font-family:Arial,Helvetica,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}
          .consultation-screen{display:none!important}
          .print-document{display:block!important;position:relative;box-sizing:border-box;width:210mm!important;height:297mm!important;max-width:210mm!important;max-height:297mm!important;overflow:hidden!important;background:#fff;color:#172033;padding:7mm 9mm 6mm}
          .print-header{height:25mm;box-sizing:border-box;border-bottom:1px solid #183d70;display:flex;align-items:center;justify-content:space-between}.print-brand{display:flex;align-items:center;gap:4mm;min-width:0}.print-logo{display:block;width:15mm;height:15mm;object-fit:contain;flex:0 0 15mm}.print-hospital{font-family:Georgia,'Times New Roman',serif;font-size:15px;font-weight:700;letter-spacing:.04em;color:#082b61;line-height:1.15}.print-subtitle{margin-top:1.2mm;font-size:8px;letter-spacing:.16em;text-transform:uppercase;color:#64748b;font-weight:700}.print-meta{text-align:right;font-size:8px;line-height:1.5;font-weight:800;color:#082b61;white-space:nowrap;border:1px solid #d7c08a;border-radius:2mm;padding:2mm 3mm}
          .print-patient{height:25mm;box-sizing:border-box;display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid #cbd5e1;padding:3mm 0;gap:2.5mm 5mm;overflow:hidden}.print-patient span,.print-footer span{display:block;font-size:6.7px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:#64748b}.print-patient strong{display:block;font-size:8.3px;line-height:1.15;margin-top:.8mm;color:#172033;overflow-wrap:anywhere}
          .print-complaints{height:25mm;box-sizing:border-box;display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid #cbd5e1;overflow:hidden}.print-complaints>div{padding:3mm 4mm}.print-complaints>div+div{border-left:1px solid #cbd5e1}.print-complaints h3,.print-section h3{margin:0 0 1.5mm;font-size:8px;letter-spacing:.12em;text-transform:uppercase;color:#082b61;font-weight:900}.print-complaints p,.print-section p{margin:0;font-size:8.2px;line-height:1.3;white-space:pre-wrap;overflow-wrap:anywhere}
          .print-body{height:205mm;box-sizing:border-box;display:grid;grid-template-columns:39mm 1fr;border-bottom:1px solid #cbd5e1;overflow:hidden}.print-sidebar{border-right:1px solid #cbd5e1;padding:3.5mm 3.5mm;overflow:hidden}.print-tool{padding:3mm 0;border-bottom:1px solid #e2e8f0}.print-tool:last-child{border-bottom:0}.print-tool b{display:block;font-size:8px;color:#082b61;font-weight:900;margin-bottom:1mm}.print-tool p{margin:0;font-size:7.5px;line-height:1.3;white-space:pre-wrap;overflow-wrap:anywhere}.print-main{position:relative;min-width:0;padding:3.5mm 4.5mm 28mm;overflow:hidden}.print-section{border-bottom:1px solid #e2e8f0;padding-bottom:3mm;margin-bottom:3mm}.print-section table{width:100%;border-collapse:collapse;table-layout:fixed}.print-section th,.print-section td{border-bottom:1px solid #e2e8f0;padding:1.4mm 1.5mm;text-align:left;font-size:7.5px;vertical-align:top;overflow-wrap:anywhere}.print-section th{font-size:7px;color:#64748b;text-transform:uppercase;letter-spacing:.06em}.print-empty{font-size:8px;color:#94a3b8;padding:3mm 0}.print-footer{position:absolute;left:4.5mm;right:4.5mm;bottom:4mm;display:flex;justify-content:space-between;align-items:flex-end;border-top:1px solid #cbd5e1;padding-top:3mm}.print-footer strong{display:block;font-size:9px;margin-top:1.2mm;color:#082b61}.print-sign{text-align:center;min-width:45mm}.print-sign em{display:block;font-style:normal;font-size:7px;color:#64748b;margin-top:6mm;border-top:1px solid #64748b;padding-top:1mm}
        }
      `}</style>
    </>
  );
