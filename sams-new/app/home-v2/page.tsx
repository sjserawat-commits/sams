...
export default function HomeV2() {
  ...
  const hi = lang === "hi";
  return (
    <main className="min-h-screen bg-[#f7f8fb] text-[#10233f]">
      <header className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-3 sm:px-8">
          <Link href="/home-v2" className="flex items-center gap-3">
            <Image src="/serawat-logo.png" alt="SAMS" width={76} height={44} className="h-11 w-auto object-contain" priority />
            <div><p className="font-serif text-xl font-bold tracking-tight text-[#082b61]">SAMS</p><p className="hidden text-[9px] font-bold uppercase tracking-[0.16em] text-slate-400 sm:block">Serawat Advanced Musculoskeletal, Joint & Spine Centre</p></div>
          </Link>
          <div className="flex items-center gap-3"><div className="flex rounded-full border border-slate-200 bg-slate-50 p-1 text-[10px] font-black"><button onClick={() => setLang("en")} className={`rounded-full px-3 py-1.5 ${!hi ? "bg-[#082b61] text-white" : "text-slate-500"}`}>EN</button><button onClick={() => setLang("hi")} className={`rounded-full px-3 py-1.5 ${hi ? "bg-[#082b61] text-white" : "text-slate-500"}`}>हिन्दी</button></div><Link href="/dashboard" className="hidden rounded-xl bg-[#082b61] px-4 py-2.5 text-xs font-black text-white sm:block">Command Center ↗</Link></div>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-[1200px] px-5 py-16 sm:px-8 sm:py-24">
        <div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:items-start"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#0b63ce]">A message from your doctor</p><h2 className="mt-4 font-serif text-4xl font-medium leading-tight text-[#082b61] sm:text-5xl">{hi ? "हर मरीज की कहानी अलग है।" : "Every patient has a different story."}</h2></div><div className="border-l border-slate-200 pl-6 sm:pl-10"><p className="font-serif text-2xl leading-[1.45] text-[#203b5d] sm:text-3xl">{hi ? "SAMS में हम पहले आपकी कहानी समझते हैं—आपकी समस्या, आपकी गतिविधि और आपके लक्ष्य। हमारा उद्देश्य केवल बीमारी का उपचार नहीं, बल्कि बेहतर गति, बेहतर कार्यक्षमता और अधिक स्वतंत्रता की ओर आपकी यात्रा में साथ देना है।" : "At SAMS, we understand your story first — your problem, your movement, your daily life and your goals. Our aim is not simply to treat a condition, but to help you move better, function better and live better."}</p><p className="mt-7 text-xs font-black uppercase tracking-[0.16em] text-slate-400">— Dr. Suraj Serawat</p></div></div>
      </section>

      <section className="border-y border-slate-200 bg-white"><div className="mx-auto max-w-[1200px] px-5 py-16 sm:px-8 sm:py-24"><div className="max-w-3xl"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#0b63ce]">Why SAMS</p><h2 className="mt-4 font-serif text-4xl font-medium leading-tight text-[#082b61] sm:text-6xl">{hi ? "देखभाल जो निदान से आगे देखती है।" : "Care that sees beyond the diagnosis."}</h2><p className="mt-5 text-base leading-8 text-slate-500">{hi ? "SAMS में विशेषज्ञ मूल्यांकन, दर्द प्रबंधन, पुनर्वास और कार्यात्मक सुधार को व्यक्ति-केंद्रित योजना में जोड़ा जाता है।" : "SAMS brings specialist assessment, pain medicine, rehabilitation and functional recovery together around one personalised plan — because meaningful recovery is about more than a diagnosis."}</p></div><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{["Personalised assessment","Evidence-informed care","Goal-oriented rehabilitation","Continuity & follow-up"].map((x,i)=><div key={x} className="rounded-2xl border border-slate-200 p-6"><span className="font-serif text-3xl text-[#0b63ce]">0{i+1}</span><h3 className="mt-5 text-sm font-black text-[#082b61]">{x}</h3><p className="mt-2 text-xs leading-5 text-slate-400">{["Understand the person, not only the condition.","Treatment shaped by clinical reasoning and evidence.","Plans built around movement, function and independence.","Support beyond the first appointment."][i]}</p></div>)}</div></div></section>

      <section className="mx-auto max-w-[1400px] px-5 py-16 sm:px-8 sm:py-24"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#0b63ce]">Our areas of expertise</p><h2 className="mt-4 font-serif text-4xl font-medium text-[#082b61] sm:text-6xl">{hi ? "पुनर्वास विशेषज्ञता का व्यापक दायरा" : "Expertise across the spectrum of rehabilitation."}</h2></div><p className="max-w-md text-sm leading-6 text-slate-500">{hi ? "दर्द और जोड़ से लेकर न्यूरोलॉजिकल एवं बाल पुनर्वास तक।" : "From pain and joints to neurological, paediatric and spinal rehabilitation — specialised care, connected."}</p></div><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{fields.map(f=><article key={f.n} className="group rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_12px_40px_rgba(8,43,97,0.04)] transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_22px_60px_rgba(8,43,97,0.10)]"><div className="flex items-start justify-between"><span className="font-serif text-3xl text-[#0b63ce]/70">{f.n}</span><span className="rounded-full bg-blue-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-[#0b63ce]">SAMS</span></div><h3 className="mt-8 font-serif text-2xl font-medium leading-tight text-[#082b61]">{hi ? f.hi : f.en}</h3><p className="mt-3 text-xs font-black uppercase tracking-[0.08em] text-[#0b63ce]">{f.tag}</p><p className="mt-4 text-xs leading-6 text-slate-500">{f.items}</p><div className="mt-6 border-t border-slate-100 pt-4 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400 group-hover:text-[#0b63ce]">Explore care →</div></article>)}</div></section>
      ...
      <footer className="border-t border-slate-200 bg-white"><div className="mx-auto flex max-w-[1200px] flex-col gap-2 px-5 py-8 text-xs text-slate-400 sm:px-8 md:flex-row md:justify-between"><span>SAMS • Serawat Advanced Musculoskeletal, Joint & Spine Centre</span><span>Clinical care • Pain medicine • Rehabilitation</span></div></footer>
    </main>
  );
}
