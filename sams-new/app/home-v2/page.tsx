            ...
            <div className="rounded-[1.35rem] bg-slate-50 p-5 sm:p-6">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                {actions.map(([label, description, href, icon, tone]) => tone === "primary" ? (
                  <Link key={href} href={href} className="group relative overflow-hidden rounded-2xl border border-blue-100 bg-white p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-lg hover:shadow-blue-900/10">
                    <div className="absolute inset-y-0 left-0 w-1 bg-[#0b63ce]" />
                    <div className="flex items-center gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#0b63ce] text-lg font-bold text-white shadow-md shadow-blue-900/20">{icon}</span><span className="min-w-0"><span className="block text-sm font-bold text-[#082b61]">{label}</span><span className="mt-0.5 block text-xs font-medium text-slate-400">{description}</span></span><span className="ml-auto text-lg text-slate-300 transition-transform group-hover:translate-x-1 group-hover:text-[#0b63ce]">→</span></div>
                  </Link>
                ) : (
                  <Link key={href} href={href} className="group rounded-2xl border border-white bg-white px-4 py-3.5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-100 hover:shadow-md">
                    <div className="flex items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-sm font-bold text-[#0b63ce] transition-colors group-hover:bg-[#0b63ce] group-hover:text-white">{icon}</span><span className="min-w-0"><span className="block text-sm font-bold text-slate-700 group-hover:text-[#0b63ce]">{label}</span><span className="block truncate text-[11px] font-medium text-slate-400">{description}</span></span><span className="ml-auto text-slate-300 transition-all group-hover:translate-x-1 group-hover:text-[#0b63ce]">→</span></div>
                  </Link>
                ))}
              </div>
            </div>
