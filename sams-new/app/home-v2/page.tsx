...
export default function HomeV2() {
  ...
  const cards = [
    ["Patients Today", "Patients registered today", summary ? String(summary.patientsToday) : "—", "♙", "Live activity"],
    ["Encounters Today", "Clinical encounters today", summary ? String(summary.encountersToday) : "—", "✚", "Clinical activity"],
    ["Diagnoses", "Encounters with diagnosis", summary ? String(summary.diagnosesToday) : "—", "✓", "Documentation"],
    ["System Status", "Core SAMS services", summary?.systemStatus ?? "Checking…", "●", "Platform health"],
  ];
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-[1500px] space-y-8 px-5 py-8 sm:px-8 sm:py-10">
        ...
        <section>
          <div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#0b63ce]">Today at a glance</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-[#082b61]">Clinical command center</h2></div><span className="hidden rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-400 shadow-sm sm:inline-flex">LIVE OVERVIEW</span></div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {cards.map(([title, note, value, icon, label], index) => <div key={title} className="group relative overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-blue-100 hover:shadow-xl hover:shadow-slate-200/60">
              <div className={`absolute inset-x-0 top-0 h-1 ${index === 3 ? "bg-emerald-500" : "bg-[#0b63ce]"}`} />
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0"><p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p><p className="mt-2 text-sm font-semibold text-slate-600">{title}</p><p className="mt-3 text-4xl font-black tracking-tight text-[#082b61] transition-transform duration-200 group-hover:scale-[1.03] origin-left">{value}</p></div>
                <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-lg font-bold shadow-sm ${index === 3 ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-[#0b63ce]"}`}>{icon}</span>
              </div>
              <div className="mt-5 flex items-center gap-2 border-t border-slate-100 pt-3"><span className={`h-1.5 w-1.5 rounded-full ${index === 3 ? "bg-emerald-500" : "bg-[#0b63ce]"}`} /><p className="text-[11px] font-medium text-slate-400">{note}</p></div>
            </div>)}
          </div>
        </section>
