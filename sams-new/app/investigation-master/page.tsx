      ...
      <div className="mx-auto max-w-[1280px]">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-[1.6rem] border border-slate-200 bg-white px-5 py-4 shadow-[0_12px_35px_rgba(8,43,97,0.06)] sm:px-7">
          <div className="flex items-center gap-3">
            <Link href="/investigations" className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-lg font-black text-[#082b61] shadow-sm">←</Link>
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#0b63ce]">SAMS · Diagnostics Administration</p>
              <h1 className="mt-1 text-xl font-black tracking-tight text-[#082b61]">Investigation Master</h1>
            </div>
          </div>
          <button onClick={() => setShowForm((v) => !v)} className="rounded-xl bg-[#082b61] px-4 py-2.5 text-xs font-black text-white shadow-sm">{showForm ? "Close" : "+ Add Investigation"}</button>
        </header>
        ...
        <section className="mb-5 overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#082b61] via-[#075dcc] to-[#0b63ce] p-7 text-white shadow-[0_25px_70px_rgba(8,43,97,0.18)] sm:p-9">
          ...
          <div className="mt-3 flex flex-wrap items-end justify-between gap-5">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-blue-100">Central catalogue for OPD advice, investigation billing, investigation room and reports. Search supports investigation names, abbreviations, codes and aliases.</p>
            </div>
            <div className="rounded-2xl border border-white/20 bg-white/10 px-5 py-4 backdrop-blur-sm"><p className="text-2xl font-black">{rows.length}</p><p className="text-[9px] font-black uppercase tracking-widest text-blue-100">Active matches</p></div>
          </div>
        </section>

        {showForm && <CreateInvestigation onCreated={() => { setShowForm(false); load(); }} />}
        ...
        <section className="rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-[0_15px_40px_rgba(8,43,97,0.06)]">
          <div className="flex flex-col gap-3 md:flex-row">
            ...
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none">
          ...
          <div className="mt-4 flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-slate-400">
            <span>{rows.length} results · {categoryCount} categories in current result</span>
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-slate-100">
            <table className="w-full min-w-[900px] text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {!loading && rows.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-400">No active investigations found.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
...
function CreateInvestigation({ onCreated }: { onCreated: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch("/api/investigation-master", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, category, code, rate, aliases }) });
      const data = await response.json();
  ...
  return <section className="mb-5 rounded-[1.7rem] border border-blue-100 bg-white p-5 shadow-[0_15px_40px_rgba(8,43,97,0.06)]"><div className="grid gap-3 md:grid-cols-4"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Investigation name" className="rounded-xl border border-slate-200 px-4 py-3 text-sm" /><select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3 text-sm">{categories.map((item) => <option key={item}>{item}</option>)}</select><input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Code (optional)" className="rounded-xl border border-slate-200 px-4 py-3 text-sm" /><input value={rate} onChange={(e) => setRate(e.target.value)} type="number" min="0" placeholder="Rate" className="rounded-xl border border-slate-200 px-4 py-3 text-sm" /><input value={aliases} onChange={(e) => setAliases(e.target.value)} placeholder="Search aliases, comma separated" className="rounded-xl border border-slate-200 px-4 py-3 text-sm md:col-span-3" /><button disabled={saving || !name.trim()} onClick={save} className="rounded-xl bg-[#0b63ce] px-4 py-3 text-sm font-black text-white disabled:opacity-50">{saving ? "Saving..." : "Save Investigation"}</button></div>{error && <p className="mt-3 text-xs font-semibold text-red-600">{error}</p>}</section>;
}
