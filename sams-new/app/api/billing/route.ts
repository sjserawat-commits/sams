...
export async function POST(request: Request) {
  try {
    ...
    console.error("Billing write error:", error);
    return NextResponse.json({ error: "Unable to update billing." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body?.action === "discount") {
      ...
      const netAmount = round(bill.subtotal - discount);
      const updated = await prisma.billingRecord.update({ where: { id }, data: { discount: round(discount), netAmount, balanceAmount: netAmount } });
