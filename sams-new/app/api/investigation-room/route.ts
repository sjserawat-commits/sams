...
const ALLOWED_STATUS = ["ORDERED", "SAMPLE_COLLECTED", "PROCESSING", "COMPLETED", "CANCELLED"] as const;
  ...
  try {
    ...
    const orders = await prisma.investigationOrder.findMany({
      where: {
        ...
        ...(status && ALLOWED_STATUS.includes(status as InvestigationStatus)
          ? { status }
          : {}),
        ...(q
          ? {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!ALLOWED_STATUS.includes(status as InvestigationStatus)) {
    ...
    if (status === "COMPLETED") {
      if (!reportText) return NextResponse.json({ error: "Report/result is required before completing the investigation." }, { status: 400 });
      const bill = order.opdVisit.billingRecords[0];
      if (bill && bill.balanceAmount > 0) {
        return NextResponse.json({ error: "Investigation report cannot be finalized while the OPD bill has an outstanding balance." }, { status: 400 });
      }
    }
    ...
    return NextResponse.json({ error: "Unable to update investigation result." }, { status: 500 });
  }
}
