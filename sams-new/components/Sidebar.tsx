...
function NavItem({ href, label, icon }: { href: string; label: string; icon: string }) {
  return (
    <Link
      href={href}
      className="group flex w-full min-w-0 items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 text-sm font-bold text-slate-300 transition-all hover:border-[#d6a443]/30 hover:bg-white/5 hover:text-[#f2d38b]"
    >
      ...
      <div className="!block min-w-0 flex-1 overflow-visible whitespace-nowrap text-sm font-bold leading-5 text-slate-300 group-hover:text-[#f2d38b]">
        {label}
      </div>
    </Link>
  );
...
export default function Sidebar() {
  return (
    <aside className="sticky top-0 z-40 hidden h-screen w-[300px] min-w-[300px] shrink-0 flex-col overflow-y-auto overflow-x-visible border-r border-[#d6a443]/20 bg-[linear-gradient(180deg,#061525_0%,#0a1d31_48%,#071525_100%)] px-4 py-5 shadow-[12px_0_40px_rgba(0,0,0,0.28)] lg:flex">
      <div className="relative mb-5 min-w-0 overflow-hidden rounded-[1.25rem] border border-[#d6a443]/35 bg-[linear-gradient(135deg,#0b3158,#071b30)] p-4 text-white shadow-[0_16px_35px_rgba(0,0,0,0.25)]">
        ...
        <Link href="/" className="relative flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="block text-lg font-black tracking-tight text-[#f2d38b]">SAMS</div>
            <div className="mt-1 block text-[8px] font-bold uppercase leading-3 tracking-[0.11em] text-slate-300">
      ...
      <nav className="mb-5 w-full space-y-1">
      ...
      <div className="mt-auto pt-5">
        <div className="rounded-[1.25rem] border border-[#d6a443]/20 bg-[#071525]/80 p-4 shadow-inner">
          <div className="text-[9px] font-black uppercase tracking-[0.2em] text-[#d6a443]">Front Desk</div>
          <div className="mt-1 text-sm font-black text-white">Patient journey starts here</div>
          <div className="mt-1 text-[11px] leading-4 text-slate-400">Registration → OPD → Consultation → Care → Follow-up</div>
        </div>
      </div>
    </aside>
  );
